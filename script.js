...
const mapData = [
    ...
    { id: 7, name: "Absolute Industrial Solutions", desc: "Industry Internship (2025).", x: 55, y: 65 },
...
window.onload = function() { simulateLoading(); };

function simulateLoading() {
    const bar = document.getElementById('progressBar');
    const ball = document.getElementById('rollingPball');
    ...
    let interval = setInterval(() => {
        if (width >= 100) {
            ...
            ball.style.transform = `translateX(-50%) rotate(${width * 10}deg)`;
        }
    }, 30);
}
...
function startApp() {
    ...
    if (soundLevels[currentSoundIndex] !== "OFF") {
        if(currentSoundIndex === 0) currentSoundIndex = 2; 
...
function renderSidePanelScreen(type, content, header) {
    if(type === 'MAP') {
        ...
        content.innerHTML = `<div class="map-screen-wrapper" onclick="closePopup()">
            <div class="map-label">MAP</div>
            <div class="map-landmass" style="width: 15%; height: 10%; top: 5%; left: 40%; border-radius: 20px;"></div>
            <div class="map-landmass" style="width: 82%; height: 65%; top: 32%; left: -2%; border-radius: 0 120px 100px 0;"></div>
            <div class="map-landmass" style="width: 20%; height: 25%; top: 5%; left: 82%; border-radius: 40px;"></div>
            <div class="map-container">
                <div class="map-route-v" style="left: 10%; top: 65%; height: 15%;"></div>
                <div class="map-route-h" style="left: 10%; top: 65%; width: 15%;"></div>
                <div class="map-route-v" style="left: 25%; top: 50%; height: 15%;"></div>
                <div class="map-route-h" style="left: 25%; top: 50%; width: 15%;"></div>
                <div class="map-route-h" style="left: 40%; top: 50%; width: 15%;"></div>
                <div class="map-route-v" style="left: 55%; top: 50%; height: 15%;"></div>
                <div class="map-route-h" style="left: 55%; top: 65%; width: 15%;"></div>
                <div class="map-route-v route-bridge" style="left: 70%; top: 50%; height: 15%;"></div>
                <div class="map-route-h route-bridge" style="left: 70%; top: 50%; width: 18%;"></div>
                <div class="map-route-v route-bridge" style="left: 88%; top: 15%; height: 35%;"></div>
                <div class="map-route-h route-bridge" style="left: 88%; top: 15%; width: 6%;"></div>
                ${decoHTML}${nodesHTML}
...
function renderBeltScreen(type) {
    ...
    container.innerHTML = ""; 
    let bgClass = "bg-" + type.toLowerCase();
    if(type === 'CERTIFICATE') bgClass = "bg-certificate";
    ...
    if (type === 'PARTY') {
        ...
        let slotsHTML = skills.map(s => `<div class="party-slot"><div class="slot-icon">${s.symbol}</div><div class="slot-info"><div class="slot-header"><span class="slot-name">${s.name}</span><span class="slot-level">Lv. ${s.lv}</span></div><div class="hp-container"><div class="hp-bar" style="width: ${s.exp}%; background: ${s.exp > 50 ? '#78c850' : '#f8d030'}"></div><div class="hp-text">${s.exp}/100 PRO</div></div></div></div>`).join('');
        container.innerHTML = `<div class="party-screen-full ${bgClass}">${titleHTML}${slotsHTML}<div class="bag-item-row" style="position:absolute; bottom:80px; right:40px; background:#f85858; color:white; border:4px solid #fff; border-radius:10px; width:150px; justify-content:center; box-shadow: 4px 4px 0 #000;" onclick="resetToMain()"><span class="select-arrow" style="visibility:visible; color:white;">▶</span> CLOSE</div></div>`;
        return;
    }

    if(type === 'SETTINGS') {
        container.innerHTML = `<div class="party-screen-full bg-settings"><div class="bag-wrapper"><div class="bag-visual-panel" style="height: 100px; flex: none;"><h2 style="font-size:3.5rem; color:#fff; text-shadow:2px 2px #000;">OPTION</h2></div><div class="settings-menu"><div class="settings-row" onclick="updateSetting('SOUND')"><span>SOUND</span><span class="settings-value" id="soundVal">${soundLevels[currentSoundIndex]}</span></div><div class="settings-row"><span>BUTTON MODE</span><span class="settings-value">HELP</span></div><div class="settings-row"><span>FRAME</span><span class="settings-value">TYPE 1</span></div><div class="settings-row" style="color:#f85858; border:none;" onclick="resetToMain()"><span>CANCEL</span></div></div></div></div>`;
        return;
    }

    let listData = type === 'PROJECT' ? projectData : credentialData;
    let listHTML = listData.map((item, i) => `<div class="bag-item-row ${i === 0 ? 'selected' : ''}" onclick="updateBagDetail(this, '${item.desc}', '${item.icon}')"><span class="select-arrow">▶</span> ${item.name}</div>`).join('');
    container.innerHTML = `<div class="party-screen-full ${bgClass}"><div class="bag-wrapper"><div class="bag-top-row"><div class="bag-visual-panel"><div class="bag-main-icon" id="bagLargeIcon"><img src="${listData[0].icon}" style="width:120px; height:120px; object-fit:contain;"></div><h2 style="font-size:2.5rem; color:#fff; text-shadow:2px 2px #000;">${type}</h2></div><div class="bag-list-panel">${listHTML}<div class="bag-item-row" onclick="resetToMain()"><span class="select-arrow">▶</span> CLOSE</div></div></div><div class="bag-description-box" id="bagDescText">${listData[0].desc}</div></div></div>`;
}
